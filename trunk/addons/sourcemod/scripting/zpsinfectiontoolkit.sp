...
public OnPluginStart() {
	...
	// zie_disinfectplayer <playerid>
	RegAdminCmd("zit_disinfectplayer", onCmdDisinfectPlayer, ADMFLAG_GENERIC,
		"Disinfect a player"
	);
	RegAdminCmd("zit_checkup", onCmdCheckup, ADMFLAG_GENERIC,
...
public Action:onCmdCheckup(client, args) {
	...
	ent = GetClientOfUserId(StringToInt(buf));
...
public Action:onCmdInfectPlayer(client, args) {
	...
	new Float:seconds;
	GetCmdArg(1, buf, sizeof(buf));
	targetEnt = GetClientOfUserId(StringToInt(buf));
	GetCmdArg(2, buf, sizeof(buf));
	seconds = StringToFloat(buf);
	ZIT_InfectPlayerInXSeconds(targetEnt, seconds);
	return Plugin_Handled;
}
